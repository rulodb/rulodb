...
use crate::storage::{
    Document, DocumentKey, Result as StorageResult, StorageBackend, StorageError,
};
...
impl<'a> Evaluator<'a> {
    ...
    async fn evaluate_lazy(
        ...
        match plan {
            PlanNode::ScanTable { name } => {
                let stream = self.storage.scan_table(name).await.map_err(EvalError::from);
                Some(stream)
            }
            _ => None,
        }
    ...
    fn evaluate<'b>(
        ...
        Box::pin(async move {
            match plan {
                ...
                PlanNode::ScanTable { name } => self.eval_scan_table(name).await,
                PlanNode::CreateTable { name, .. } => self.eval_create_table(name).await,
                PlanNode::DropTable { name } => self.eval_drop_table(name).await,
                PlanNode::ListTables => self.eval_list_tables().await,
                PlanNode::GetByKey { table, key, .. } => self.eval_get(table, key).await,
                PlanNode::Filter {
                    source, predicate, ..
    ...
    async fn eval_scan_table(&mut self, name: &str) -> Result<Datum, EvalError> {
        let mut stream = self
            .storage
            .scan_table(name)
            .await
    ...
    async fn eval_create_table(&self, name: &str) -> Result<Datum, EvalError> {
        self.storage.create_table(name).await?;
        Ok(Datum::Null)
    }

    async fn eval_drop_table(&self, name: &str) -> Result<Datum, EvalError> {
        self.storage.drop_table(name).await?;
        Ok(Datum::Null)
    }

    async fn eval_list_tables(&mut self) -> Result<Datum, EvalError> {
        let tables = self.storage.list_tables().await?;
        let mut result = Vec::new();

        for table in tables {
            self.stats.read_count += 1;
            result.push(Datum::String(table));
        }

        Ok(Datum::Array(result))
    }

    async fn eval_get(&mut self, table: &str, key: &Datum) -> Result<Datum, EvalError> {
        let Datum::String(key) = key else {
            return Err(EvalError::InvalidKeyType);
        };

        if let Some(doc) = self.storage.get(table, key).await? {
            self.stats.read_count = 1;
            Ok(Datum::Object(doc))
        } else {
            Ok(Datum::Null)
    ...
    async fn eval_insert(
        ...
        let PlanNode::ScanTable { name } = table else {
        ...
        if batch_docs.len() > 1 {
            if self.storage.put_batch(name, &batch_docs).await.is_err() {
            ...
            if self.storage.put(name, &key, &obj).await.is_err() {
                self.stats.error_count += 1;
            } else {
    ...
    async fn eval_delete(&mut self, source: &PlanNode) -> Result<Datum, EvalError> {
        ...
        if let Some(Ok(mut stream)) = self.evaluate_lazy(source).await {
            while let Some(doc) = stream.next().await {
                if let Ok(doc) = doc {
                    let table = get_document_key_value(&doc, &DocumentKey::Table)?;
                    let key = get_document_key_value(&doc, &DocumentKey::Id)?;

                    if self.storage.delete(&table, &key).await.is_ok() {
                        deleted += 1;
                    } else {
                        errors += 1;
                    }
...
fn get_document_key_value(doc: &Document, key: &DocumentKey) -> Result<String, EvalError> {
